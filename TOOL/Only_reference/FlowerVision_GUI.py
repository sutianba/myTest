...
class FlowerVisionAI:
    ...
    def load_model(self, weights_path=None):
        ...
        try:
            ...
            if self.verbose:
                print(f"加载模型失败: {e!s}")
            raise RuntimeError(f"加载模型失败: {e!s}")
    ...
    def recognize(self, img_path, conf_thres=0.25, iou_thres=0.45):
        ...
        Returns:
            tuple: (识别结果列表, 原始图片) 识别结果列表中每个元素是包含以下键的字典：
                 - 'flower': 花卉类别名称
        ...
        try:
            ...
            raise RuntimeError(f"识别过程出错: {e!s}")
    ...
    def visualize_results(self, img, results, thickness=2, font_scale=0.5):
        ...
        for i, result in enumerate(results):
            ...
            # 绘制标签背景
            cv2.rectangle(vis_img, (bbox[0], bbox[1] - label_height - 5), (bbox[0] + label_width, bbox[1]), color, -1)

            # 绘制标签文本
            cv2.putText(
                vis_img, label, (bbox[0], bbox[1] - 5), cv2.FONT_HERSHEY_SIMPLEX, font_scale, (255, 255, 255), 1
            )
...
class FlowerVisionGUI(QMainWindow):
    ...
    def select_image(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(
            self, "选择图片文件", "", "图片文件 (*.jpg *.jpeg *.png *.tiff *.bmp)"
        )
