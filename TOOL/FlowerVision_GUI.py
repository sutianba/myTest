...
from PyQt5.QtGui import QFont, QImage, QPixmap, QTextOption
...
from tool.flower_vision_threads import AddressLookupThread, ExifProcessThread, RecognitionThread
...
class FlowerVisionGUI(QMainWindow):
    ...
    def __init__(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "初始化错误", f"加载默认模型失败: {e!s}")
            self.close()
    ...
    def init_ui(self):
        ...
        self.model_info_label.setStyleSheet(
            "color: #336699; font-weight: bold; padding: 8px; background-color: rgba(255, 255, 255, 0.8); border-radius: 8px;"
        )
        ...
        self.scroll_area.setStyleSheet(
            "background-color: rgba(255, 255, 255, 0.9); border: 1px solid #b3d1e6; border-radius: 10px; margin: 5px;"
        )
        ...
        self.image_info.setStyleSheet(
            "color: #336699; font-size: 12px; padding: 5px; background-color: rgba(255, 255, 255, 0.7); border-radius: 6px;"
        )
        ...
        exif_label.setStyleSheet(
            "font-weight: bold; padding: 5px; color: #336699; background-color: rgba(255, 255, 255, 0.7); border-radius: 6px;"
        )
        ...
        self.loading_label.setStyleSheet(
            "color: #0066cc; font-style: italic; padding: 5px; background-color: rgba(255, 255, 255, 0.7); border-radius: 6px;"
        )
    ...
    def setup_console_redirect(self):
        ...
        class ConsoleRedirector:
            ...
            def write(self, text):
                # 确保在主线程更新UI
                QApplication.instance().postEvent(self.parent, QEvent(QEvent.User), Qt.HighEventPriority)
                self.buffer += text
    ...
    def select_folder(self):
        ...
        folder_path = QFileDialog.getExistingDirectory(
            self, "选择图片文件夹", "", QFileDialog.ShowDirsOnly | QFileDialog.DontResolveSymlinks
        )
        ...
        if folder_path:
            ...
            try:
                ...
                QMessageBox.warning(self, "警告", f"读取文件夹失败: {e!s}")
    ...
    def _update_image_paths(self, file_paths):
        ...
        if file_paths:
            ...
            self.statusBar().showMessage(
                f"已选择 {len(file_paths)} 张图片，当前显示第 {self.current_image_index + 1} 张"
            )
        else:
            # 重置状态当没有选择图片时
            self.image_paths = []
    ...
    def display_image_preview(self, file_path):
        ...
        try:
            ...
            scaled_pixmap = self.original_pixmap.scaled(viewport_size, Qt.KeepAspectRatio, Qt.SmoothTransformation)
            # 设置pixmap
            self.image_label.setPixmap(scaled_pixmap)
            # 确保图片标签根据内容大小调整
            self.image_label.adjustSize()
            ...
            QMessageBox.warning(self, "警告", f"无法显示图片: {e!s}")
            ...
            if location_info:
                ...
                if location_info.get("has_location"):
                    ...
                    if location_info.get("address_info") is None:
                        self.start_address_lookup(location_info.get("decimal_lat"), location_info.get("decimal_lon"))
                else:
                    formatted_info += f"  {location_info.get('error', '未找到位置信息')}\n"

            self.exif_text.setPlainText(formatted_info)
        except Exception as e:
            self.exif_text.setPlainText(f"无法获取EXIF信息: {e!s}")
    ...
    def on_address_lookup_finished(self, address_info):
        ...
        try:
            ...
            if hasattr(self, "current_location_info"):
                ...
                self.current_location_info["formatted_address"] = (
                    self.exif_reader.format_address(address_info) if address_info else "无法获取地址信息"
                )

                # 如果当前图片已在processed_images中，更新位置信息
                if self.current_image_path and self.current_image_path in self.processed_images:
                    self.processed_images[self.current_image_path]["location"] = self.current_location_info[
                        "formatted_address"
                    ]
    ...
    def start_recognition(self):
        ...
        self.recognition_thread = RecognitionThread(
            self.flower_detector, self.current_image_path, conf_thres, iou_thres
        )
    ...
    def next_image(self):
        ...
        if hasattr(self, "processed_images") and self.current_image_path in self.processed_images:
            ...
            if results:
                # 可视化结果并显示
                vis_img = self.flower_detector.visualize_results(img, results, thickness=20, font_scale=2.0)
        ...
        self.statusBar().showMessage(
            f"已选择 {len(self.image_paths)} 张图片，当前显示第 {self.current_image_index + 1} 张"
        )
    ...
    def batch_recognize_all(self):
                ...
                try:
                    ...
                    for i, image_path in enumerate(self.image_paths):
                        ...
                        results, img = self.detector.recognize(
                            image_path, conf_thres=self.conf_thres, iou_thres=self.iou_thres
                        )

                        # 保存识别结果
                        all_results[image_path] = {"results": results, "image": img}
        ...
        # 创建并启动批量识别线程
        self.batch_thread = BatchRecognitionThread(self.flower_detector, self.image_paths, conf_thres, iou_thres)
        # 连接信号
        ...
        self.batch_thread.error.connect(
            lambda error_msg: QMessageBox.warning(self, "错误", f"批量识别失败: {error_msg}")
        )
                    ...
                    for i, (reader, img_path, loc_info) in enumerate(self.location_tasks):
                        try:
                            ...
                            address_info = reader.get_address_from_coordinates(
                                loc_info["decimal_lat"], loc_info["decimal_lon"]
                            )
    ...
    def open_classification_album(self):
        ...
        try:
                        ...
                        for result in results:
                            ...
                            if img_path not in added_images_per_flower[flower_name]:
                                ...
                                classified_data[flower_name].append(
                                    {"path": img_path, "location": location, "confidence": result.get("confidence", 0)}
                                )
                                # 记录已添加的图片路径
                                added_images_per_flower[flower_name].add(img_path)
                except Exception as e:
                    print(f"处理图片数据时出错 {img_path}: {e!s}")
                    continue
                ...
                try:
                    ...
                    if recognized_flowers:
                        ...
                        for flower in recognized_flowers:
                            ...
                            # 添加图片信息到对应花卉类别
                            classified_data[flower].append({"path": self.current_image_path, "location": location_info})
                except Exception as e:
                    print(f"提取当前图片分类数据时出错: {e!s}")
                    QMessageBox.warning(self, "警告", f"无法提取当前图片的分类数据: {e!s}")
            ...
            try:
                ...
                QMessageBox.critical(self, "导入错误", f"无法导入分类相册模块: {e!s}")
                return
            ...
            try:
                ...
                QMessageBox.critical(self, "错误", f"打开分类相册对话框时出错: {e!s}")
                print(f"打开分类相册对话框时出错: {e!s}")
        except Exception as e:
            # 捕获所有可能的异常，确保程序不会崩溃
            QMessageBox.critical(self, "严重错误", f"打开分类相册时发生严重错误: {e!s}")
            print(f"打开分类相册时发生严重错误: {e!s}")
