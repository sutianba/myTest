...
def parse_arguments():
    ...
    parser.add_argument("--subsets", type=str, default="train,val,test", help="需要处理的子集，逗号分隔")
    parser.add_argument("--label_dir", type=str, default="labelTxt", help="原始标签文件夹名称")
    # 注意：YOLOv5硬编码使用'labels'文件夹，不要修改此参数
    parser.add_argument("--output_dir", type=str, default="labels", help="输出标签文件夹名称（YOLOv5要求为labels）")
    return parser.parse_args()


def get_image_dimensions(image_path):
    """获取图像尺寸 使用PIL库读取实际图像尺寸，确保坐标归一化正确.
    """
    try:
        ...
        print(f"警告：无法读取图像 {image_path} 的尺寸，使用默认640x640。错误：{e!s}")
        return 640, 640


def convert_label_file(label_file, output_file, image_dir):
    """转换单个标签文件 根据之前查看的标签格式："144 19.00000000000001 49 19.000000000000007 49 117.00000000000001 144 117.00000000000003
    rose 0" 调整解析逻辑以适应多边形格式的标签.
    """
    ...
    with open(label_file, encoding="utf-8") as f_in:
...
if __name__ == "__main__":
    ...
    print(
        "5. 执行脚本后，请运行: python train.py --img 640 --batch 16 --epochs 300 --data ./data/flower_dataset.v15i.yolov5-obb/data.yaml --weights yolov5s.pt"
    )
