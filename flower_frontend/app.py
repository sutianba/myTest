...
def process_single_image(image_data):
    ...
    if USE_REAL_MODEL:
        ...
        for _ in range(num_flowers):
            ...
            results.append(
                {
                    "name": random.choice(
                        ["玫瑰", "郁金香", "百合", "康乃馨", "向日葵", "兰花", "菊花", "牡丹", "茉莉", "紫罗兰"]
                    ),
    ...
    for result in results:
        detection_results.append(
            {"name": result["name"], "confidence": float(result["confidence"]), "bbox": result["bbox"]}
        )

    # 返回包含识别结果和日期信息的响应
    return {"detections": detection_results, "date": image_date}
